...
class EditBoardCardForm( ttk.LabelFrame ):
    ...
    def update_tag_entries( self, *params ):
        ...
        if self.tag_key_entry.get( ).strip( ):
            if self.tag_key_entry.get( ) not in self.card_tag_keys:
                ...
                self.tag_val_entry[ "state" ] = tk.NORMAL
                self.tag_val_entry.delete( 0, tk.END )
                self.tag_val_entry[ "state" ] = tk.DISABLED
    ...
    def add_tag( self ):
        ...
        self.tag_key_entry.delete( 0, tk.END )
        self.tag_val_entry.delete( 0, tk.END )
...
def delete_project_confirm( ):
    ...
    confirm = messagebox.askyesno("Confirm Deletion", f"Are you sure you want to delete the project '{current_state[0]}'?")
    if not confirm:
    ...
    # Remove the project from the Treeview in real time
    tree_item_id = f'p{current_state[0]}'
    if tree_item_id in tree_v.get_children():
        tree_v.delete(tree_item_id)

    # Notify the user of successful deletion
    messagebox.showinfo("Deletion Successful", f"The project '{current_state[0]}' has been successfully deleted.")
...
def delete_board_confirm( ):
    ...
    confirm = messagebox.askyesno(
        "Confirm Deletion", "Are you sure you want to delete this board and its associated cards?"
    )
    if not confirm:
        return  # User canceled

    # Delete the board and its cards from the database
    manager.delete_board(current_state[1])  # Deletes board and cascades card deletions

    # Remove cards associated with the board from the UI
    project_id = f"p{current_state[0]}"  # Project ID
    board_id = f"{current_state[1]} {current_state[0]}"  # Board ID in Treeview

    # Check if the board exists in the UI
    if board_id in tree_v.get_children(project_id):
        # Remove all cards under this board in the Treeview
        children = tree_v.get_children(board_id)
        for card_id in children:
            tree_v.delete(card_id)

        # Remove the board from the Treeview
        tree_v.delete(board_id)

    # Notify user
    messagebox.showinfo("Deletion Successful", "The board and its associated cards have been deleted successfully.")
...
prompt                      = tk.Tk( )
prompt.title( "ProgressPanel" )
prompt.minsize( 250, 150 )
prompt.tk.call("source", "Sun-Valley-ttk-theme-svg/sun-valley.tcl")
...
root = tk.Tk( )
root.title( "ProgressPanel" )
root.minsize( 250, 150 )
root.tk.call("source", "Sun-Valley-ttk-theme-svg/sun-valley.tcl")
...
style = ttk.Style()
style.configure("Deleted.Treeview", foreground="red")
